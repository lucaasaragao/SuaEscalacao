...
const allPlayers: Player[] = [
  ...
  { id: 58, name: "Rodrygo", position: ["RW", "LW", "ST"], photo: "/jogadores/rodrygo.png" },
  ...
  { id: 43, name: "Luiz Henrique", position: ["RW", "LW"], photo: "/jogadores/564328_luiz_henrique_20250218163216.jpg" },
...
function getPlayerPhoto(player: Player) {
  return player.photo || getFallbackAvatar(player.name);
}

type Lineup = { [key: string]: Player | null };
const initialLineup: Lineup = {
  GK: null, LB: null, CB1: null, CB2: null, RB: null,
  CM1: null, CM2: null, CAM: null, LW: null, ST: null, RW: null,
};

type LineupAppProps = { convocatedPlayers: ConvocationPlayer[] };

function LineupApp({ convocatedPlayers }: LineupAppProps) {
  ...
  // Use convocated players if available, fallback to all players
  const sourcePool: Player[] = convocatedPlayers.length > 0
    ? convocatedPlayers.map((cp) => ({
        id: cp.id,
        name: cp.name,
  ...
  async function handleShare() {
    ...
    setIsSharing(true);
    try {
      const canvas = await html2canvas(fieldRef.current, { backgroundColor: "#fff", scale: 2, useCORS: true });
  ...
  function autoAssignPlayer(player: Player) {
    if (Object.values(lineup).some((p) => p?.id === player.id)) {
      alert(`${player.name} já está na escalação!`);
      return;
    }
    setLineup((prev) => {
      if (selectedSlotKey) return { ...prev, [selectedSlotKey]: player };
      const posArray = Array.isArray(player.position) ? player.position : [player.position];
      const slotOrder: Record<string, string[]> = {
        GK: ["GK"], CB: ["CB1", "CB2"], LB: ["LB"], RB: ["RB"],
        CM: ["CM1", "CM2"], CAM: ["CAM"], LW: ["LW"], RW: ["RW"], ST: ["ST"],
      };
      let slot: string | undefined;
      for (const pos of posArray) {
        slot = (slotOrder[pos] || []).find((k) => !prev[k]);
        if (slot) break;
      }
      if (!slot) { window.alert(`Nenhuma vaga livre para ${posArray.join(" / ")}`); return prev; }
      return { ...prev, [slot]: player };
    });
    setFilterPosition(null); setSelectedSlotKey(null); setSearch("");
  }
  ...
  function handleSlotClick(slotKey: string) {
    ...
    if (pos) setFilterPosition((prev) => (prev === pos ? null : pos));
  }

  function handleAddButtonClick(slotKey: string) {
    if (lineup[slotKey]) { setLineup((prev) => ({ ...prev, [slotKey]: null })); return; }
    const pos = mapSlotToPosition(slotKey);
    if (pos) { setFilterPosition(pos); setSelectedSlotKey(slotKey); }
  }

  // Filtered players for sidebar
  const filteredPlayers = sourcePool.filter((p) => {
    const matchesPos = !filterPosition || (
      Array.isArray(p.position) ? p.position.includes(filterPosition) : p.position === filterPosition
    );
  ...
  const renderSlot = (slotKey: string, extraClasses: string) => {
    const player = lineup[slotKey] as Player | null;
    return (
      <div className={`player-slot ${extraClasses} ${player ? "assigned" : "empty"}`} onClick={() => handleSlotClick(slotKey)}>
        {player ? (
          <>
            <img src={getPlayerPhoto(player)} alt={player.name} className="slot-photo" crossOrigin="anonymous"
              onError={(e) => { e.currentTarget.onerror = null; e.currentTarget.src = getFallbackAvatar(player.name); }} />
            <span className="slot-name-bg">{player.name}</span>
          </>
        ) : ""}
        {!isSharing && (
          <button className="add-btn" onClick={(e) => { e.stopPropagation(); handleAddButtonClick(slotKey); }}
            aria-label={player ? `Remover ${player.name}` : `Adicionar ${slotKey}`}>
            {player ? "×" : "+"}
          </button>
        )}
      </div>
  ...
  const positionLabels: Record<string, string> = {
    ...
    CM: "Meio-campo", CAM: "Meia Of.", LW: "Ponta Esq.", RW: "Ponta Dir.", ST: "Atacante",
  };

  return (
    <>
      <div className="lineup-header-controls">
        <select id="formation" value={formation} onChange={(e) => setFormation(e.target.value)} className="formation-select">
          <option value="433">4-3-3</option>
          <option value="4231">4-2-3-1</option>
          <option value="442">4-4-2</option>
        </select>
        {convocatedPlayers.length > 0 && (
          <span className="lineup-source-badge">🇧🇷 Mostrando seus 26 convocados</span>
        )}
        <button onClick={handleShare} className="btn-share" disabled={isSharing}>
      ...
      <div className="content">
        ...
        <div className={`field formation-${formation}`} ref={fieldRef}>
          <div className="pitch-area-top" /><div className="pitch-circle" /><div className="pitch-area-bottom" />
          <div className="pitch">
            <div className="row attack">{renderSlot("LW", "lw")}{renderSlot("ST", "st")}{renderSlot("RW", "rw")}</div>
            <div className="row midfield">{renderSlot("CM1", "cm1")}{renderSlot("CAM", "cam")}{renderSlot("CM2", "cm2")}</div>
            <div className="row defense">{renderSlot("LB", "lb")}{renderSlot("CB1", "cb1")}{renderSlot("CB2", "cb2")}{renderSlot("RB", "rb")}</div>
            <div className="row goal">{renderSlot("GK", "gk")}</div>
          </div>
        </div>
        ...
        <div className="sidebar">
          <div className="panel">
            {/* SEARCH BAR — Cartola style */}
            <div className="lineup-search-wrap">
              <span className="lineup-search-icon">🔍</span>
              <input
                className="lineup-search-input"
                type="search"
                placeholder="Buscar jogador..."
                value={search}
                onChange={(e) => { setSearch(e.target.value); if (e.target.value) setFilterPosition(null); }}
              />
              {search && (
                <button className="lineup-search-clear" onClick={() => setSearch("")}>×</button>
              )}
            </div>
            ...
            {!search && (
              <>
                <h2 className="panel-section-title">Posições</h2>
                <div className="positions-list">
                  {(["GK","CB","LB","RB","CM","CAM","LW","RW","ST"] as const).map((pos) => (
                    <div key={pos}
                      className={`list-item ${filterPosition === pos ? "selected" : ""}`}
                      onClick={() => setFilterPosition(filterPosition === pos ? null : pos)}>
                      <span className="list-item-pos-badge">{pos}</span>
                      <span>{positionLabels[pos]}</span>
                      {filterPosition === pos && <span className="list-item-arrow">→</span>}
                    </div>
                  ))}
                </div>
              </>
            ...
            {/* PLAYERS DRAWER */}
            <div className={`players-panel ${(filterPosition || search) ? "open" : ""}`}>
              <div className="players-panel-header">
                <h2>
                  {search
                    ? `Resultados: "${search}"`
                    : filterPosition ? positionLabels[filterPosition] ?? filterPosition : "Jogadores"}
                </h2>
                <button className="btn-close" onClick={() => { setFilterPosition(null); setSearch(""); }}>×</button>
              </div>
              <div className="player-list">
                {filteredPlayers.length === 0 ? (
                  <div className="player-list-empty">Nenhum jogador encontrado</div>
                ) : filteredPlayers.map((p) => (
                  <div key={p.id} className="player-card" onClick={() => autoAssignPlayer(p)}>
                    <img src={getPlayerPhoto(p)} alt={p.name} className="list-player-photo" crossOrigin="anonymous"
                      onError={(e) => { e.currentTarget.onerror = null; e.currentTarget.src = getFallbackAvatar(p.name); }} />
...
function App() {
  const [page, setPage] = useState<Page>("convocation");
  const [convocated, setConvocated] = useState<ConvocationPlayer[]>([]);

  const handleSelectionChange = useCallback((players: ConvocationPlayer[]) => {
    setConvocated(players);
  }, []);
    ...
    <div className="container">
      ...
      <header className="app-header">
        <div className="app-header-brand">
          ...
          <h1>{page === "convocation" ? "Os 26 da Copa" : "Sua Escalação"}</h1>
        </div>
        <nav className="header-nav">
          <button className={`nav-tab ${page === "convocation" ? "nav-tab-active" : ""}`} onClick={() => setPage("convocation")}>
            Convocação
            {convocated.length > 0 && <span className="nav-tab-badge">{convocated.length}</span>}
          </button>
          <button className={`nav-tab ${page === "lineup" ? "nav-tab-active" : ""}`} onClick={() => setPage("lineup")}>
            ⚽ Campo
          </button>
        </nav>
      </header>
      ...
      {page === "convocation" ? (
        <ConvocationPage
          ...
          onGoToLineup={() => setPage("lineup")}
        />
      ) : (
        <LineupApp convocatedPlayers={convocated} />
      )}

      {/* FOOTER */}
      <footer className="app-footer">
        <div className="footer-content">
          <div className="footer-copyright">© 2026 Lucas Aragão</div>
